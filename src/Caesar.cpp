	...
	if(!flago) {
		if(!flagi) {
			...
			try {
				...
				printf("%s\n", encrypt(input, shift, flags).c_str());
		...
		if(!flagi) {
			...
			out << encrypt(msg, shift, flags).c_str() << endl;
			out.close();
		} else {
			try {
				std::ifstream in(inputFile, std::ifstream::binary);
				...
				out << (encrypt(input, shift, flags).c_str()) << endl;
				out.close();
			} catch (std::fstream::failure::exception& e) {
				printf("Error: Input file not found");
				exit(EXIT_FAILURE);
...
string encrypt(string message, int shift, bool loopChars) {
